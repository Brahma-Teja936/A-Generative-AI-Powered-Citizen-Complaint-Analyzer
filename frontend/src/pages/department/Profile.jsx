...
import { Building2, Mail, UserCheck, KeyRound, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const DepartmentProfile = () => {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(res.data.message || 'Department password updated successfully.');
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
    } catch (err) {
      setErrorMsg(err.response?.data?.error || 'Failed to update password. Check current password.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h2 className="text-xl font-bold text-white">Department Operations Profile</h2>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800">
            <div className="flex items-center space-x-2 text-slate-400">
              ...
              <span>Official Department Email</span>
            </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800">
            ...
            <div className="font-mono text-slate-300">{user?.username}</div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-slate-400">
            ...
            {user?.description || 'Municipal Department Operations Unit'}
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4 text-xs">
        <div className="flex items-center space-x-2 border-b border-slate-800 pb-3">
          ...
          <h3 className="text-sm font-bold text-white">Change Department Password</h3>
        </div>
        ...
        <form onSubmit={handleChangePassword} className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-950 text-slate-100 p-2.5 rounded-xl border border-slate-800 focus:border-indigo-500 outline-none"
            />
          </div>
