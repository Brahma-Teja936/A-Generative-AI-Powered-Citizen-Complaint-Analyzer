...
export const DepartmentNotifications = () => {
  ...
  const markRead = async (id) => {
    try {
      await api.put(`/department/notifications/${id}/read`);
  ...
  const deleteNotification = async (id) => {
    try {
      await api.delete(`/department/notifications/${id}`);
  ...
  const clearAllNotifications = async () => {
    if (!window.confirm('Are you sure you want to clear all department notifications?')) {
      return;
    }
    setClearing(true);
    try {
      await api.delete('/department/notifications');
      setNotifications([]);
    } catch (err) {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-bold text-white">Department Notifications</h2>
        </div>
      ...
      <div className="glass-panel rounded-2xl border border-slate-800 divide-y divide-slate-800 overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-xs text-slate-400">Loading department updates...</div>
        ) : notifications.length === 0 ? (
            ...
            return (
              <div
                ...
                className={`p-4 flex items-start justify-between gap-3 text-xs transition ${
                  n.is_read ? 'opacity-75 hover:opacity-100 hover:bg-slate-900/40' : 'bg-indigo-950/20 hover:bg-indigo-950/40'
                }`}
              >
